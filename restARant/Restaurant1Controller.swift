...
class Restaurant1Controller: UIViewController, ARSCNViewDelegate {
    ...
    @IBAction func addBagel(_ sender: UIButton) {
        guard let node = globalNode else{
            return
        }
        for childNode in node.childNodes {
        ...
        guard let breadScene = SCNScene(named: "art.scnassets/sesame-bagel/sesame-bagel.dae") else { return }
        let breadNode = SCNNode()
        let breadSceneChildNodes = breadScene.rootNode.childNodes
    ...
    @IBAction func addFruitCake(_ sender: UIButton) {
        guard let node = globalNode else{
            return
        }
        for childNode in node.childNodes {
        ...
        guard let breadScene = SCNScene(named: "art.scnassets/fruit-cake/fruit-cake.dae") else { return }
        let breadNode = SCNNode()
        let breadSceneChildNodes = breadScene.rootNode.childNodes
